...
## 固有
- アイコンは skill で xml を使用する（Compose Material Icons は使わない）
...
- コードで対応したら、対応内容と commit hash を返信に書く（装飾なし。バッククォートやリンク・カッコで囲むのはしない。hash の前後は空白にする）
- 対応をコミットメッセージだけに留め、スレッド返信を省略しない
...
- レビューコメント: 内容を確認し対応する。対応内容はコミットメッセージに書く。あわせてレビュースレッドに返信する（PR 本体への一般コメントは書かない）
...
## ビルド・検証（方針）
- ビルドと lint はローカルで実行する
- 包括的なテストスイートはローカルでは実行しない
- テストのロジック（振る舞い・条件分岐・期待値など）を変更した場合に限り、変更対象のテストだけを絞って単体実行してよい
- 引数・型・名前・呼び出し方の変更への追随など、テストのロジックが変わらない機械的な修正だけならテストは実行しない
